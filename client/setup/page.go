...
.chk input{appearance:none;-webkit-appearance:none;width:60px;height:20px;border:1px solid var(--line);background:none;position:relative;flex:none;margin:0;padding:0;cursor:pointer;font:inherit}
.chk input::before{content:"{{SWOFF}}";position:absolute;top:0;right:8px;line-height:18px;font-size:9px;letter-spacing:.08em;color:var(--faint)}
.chk input:checked::before{content:"{{SWON}}";right:auto;left:8px;color:var(--green);text-shadow:var(--glow)}
.chk input::after{content:"";position:absolute;top:3px;left:3px;width:12px;height:12px;background:var(--dim);transition:.15s}
.chk input:checked{border-color:var(--dim)}
.chk input:checked::after{left:43px;background:var(--green);box-shadow:0 0 7px rgba(60,255,110,.55)}
