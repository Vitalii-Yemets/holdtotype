...
func init() {
	...
	settingsStrings["de"] = map[string]string{
		...
		"S_TR_HINT":    "Übersetzung durch Whisper: nach Englisch — nativ, andere Sprachen — durch erzwungene Ausgabesprache (Qualität variiert).",
		"S_TR_DEFAULT": "Immer in die Zielsprache übersetzen", "S_TR_TARGET": "Zielsprache", "S_TR_ASK": "Sprachauswahl", "S_TR_ASK_NEVER": "Nicht fragen",
		...
		"S_LLM_HINT": "Der Standardmodus läuft auf dem Haupt-Shortcut; ein Modus kann eigenen Hotkey haben. Prompt-Profile bearbeitet ein zweites neuronales Netz (offline).",
		...
		"S_HELP_HTML": "<p class=\"wh\">Wie es funktioniert</p>" +
			...
			"<li>Das Aussehen des ganzen Programms — Fenster, Leiste, Tastenkürzel-Fenster und Symbol im Infobereich — kommt aus einer Liste „Farben“ im Bereich „System“. Es gibt sechs: Grün als Standard, Bernstein, Blau, Rosa, „Editor“ (flaches Grau, ohne Schein) und „Neon“ (violett, abgerundet). Ein Aussehen ändert mehr als die Farbe: eigene Schrift, Rundungen, Rahmenstärke, Schein und Art der Bewegung. Die Wahl gilt sofort, ohne Neustart.</li>" +
			"</ul>" +
	...
	settingsStrings["fr"] = map[string]string{
		...
		"S_TR_HINT":    "Traduction par Whisper : vers l'anglais — mode natif, autres langues — langue de sortie forcée (qualité variable).",
		"S_TR_DEFAULT": "Toujours traduire vers la langue cible", "S_TR_TARGET": "Langue cible", "S_TR_ASK": "Choix de langue", "S_TR_ASK_NEVER": "Ne pas demander",
		...
		"S_LLM_HINT": "Le mode par défaut s'applique au raccourci principal ; un mode peut avoir son propre raccourci. Les profils sont réécrits par un second réseau neuronal (hors ligne).",
		...
		"S_HELP_HTML": "<p class=\"wh\">Comment ça marche</p>" +
			...
			"<li>L'apparence de tout le programme — la fenêtre, le bandeau, la fenêtre du raccourci et l'icône — vient d'une seule liste « Couleurs » dans la section « Système ». Il y en a six : le vert terminal par défaut, ambre, bleu, rose, « Éditeur » (gris plat, sans halo) et « Néon » (violet, arrondi). Une apparence change plus que la couleur : sa police, ses arrondis, l'épaisseur des bordures, le halo et le caractère des animations. Le choix s'applique aussitôt, sans redémarrage.</li>" +
			"</ul>" +
	...
	settingsStrings["es"] = map[string]string{
		...
		"S_TR_HINT":    "La traducción la hace Whisper: al inglés — modo nativo, otros idiomas — forzando el idioma de salida (calidad variable).",
		"S_TR_DEFAULT": "Traducir siempre al idioma de destino", "S_TR_TARGET": "Idioma destino", "S_TR_ASK": "Elección de idioma", "S_TR_ASK_NEVER": "No preguntar",
		...
		"S_LLM_HINT": "El modo predeterminado usa el atajo principal; un modo puede tener su propio atajo. Los perfiles los reescribe una segunda red neuronal (sin conexión).",
		...
		"S_HELP_HTML": "<p class=\"wh\">Cómo funciona</p>" +
			...
			"<li>El aspecto de todo el programa — la ventana, la barra, la ventana del atajo y el icono — sale de una sola lista «Colores» en la sección «Sistema». Hay seis: el verde de terminal por defecto, ámbar, azul, rosa, «Editor» (gris plano, sin halo) y «Neón» (violeta, redondeado). Un aspecto cambia más que el color: su tipografía, sus esquinas, el grosor de los bordes, el halo y el carácter de las animaciones. La elección se aplica al momento, sin reiniciar.</li>" +
			"</ul>" +
	...
	settingsStrings["it"] = map[string]string{
		...
		"S_TR_HINT":    "La traduzione è fatta da Whisper: verso l'inglese — modalità nativa, altre lingue — forzando la lingua di output (qualità variabile).",
		"S_TR_DEFAULT": "Traduci sempre nella lingua di destinazione", "S_TR_TARGET": "Lingua di destinazione", "S_TR_ASK": "Scelta della lingua", "S_TR_ASK_NEVER": "Non chiedere",
		...
		"S_LLM_HINT": "La modalità predefinita usa la scorciatoia principale; una modalità può avere il proprio hotkey. I profili sono riscritti da una seconda rete neurale (offline).",
		...
		"S_HELP_HTML": "<p class=\"wh\">Come funziona</p>" +
			...
			"<li>L'aspetto di tutto il programma — la finestra, la barra, la finestra della scorciatoia e l'icona — viene da un solo elenco «Colori» nella sezione «Sistema». Sono sei: il verde da terminale come predefinito, ambra, blu, rosa, «Editor» (grigio piatto, senza alone) e «Neon» (viola, arrotondato). Un aspetto cambia più del colore: il carattere tipografico, gli angoli, lo spessore dei bordi, l'alone e il modo in cui tutto si muove. La scelta vale subito, senza riavvio.</li>" +
			"</ul>" +
	...
	settingsStrings["pl"] = map[string]string{
		...
		"S_TR_HINT":    "Tłumaczy Whisper: na angielski — tryb natywny, inne języki — przez wymuszenie języka wyjściowego (jakość bywa różna).",
		"S_TR_DEFAULT": "Zawsze tłumacz na język docelowy", "S_TR_TARGET": "Język docelowy", "S_TR_ASK": "Wybór języka", "S_TR_ASK_NEVER": "Nie pytaj",
		...
		"S_LLM_HINT": "Tryb domyślny działa na głównym skrócie; tryb może mieć własny skrót. Profile przepisuje druga sieć neuronowa (offline).",
		...
		"S_HELP_HTML": "<p class=\"wh\">Jak to działa</p>" +
			...
			"<li>Wygląd całego programu — okna, paska, okna skrótu i ikony w zasobniku — bierze się z jednej listy „Kolory” w sekcji „System”. Jest ich sześć: zielony terminalowy domyślnie, bursztynowy, niebieski, różowy, „Edytor” (płaska szarość, bez poświaty) i „Neon” (fiolet, zaokrąglony). Wygląd zmienia więcej niż kolor: własny krój pisma, zaokrąglenia, grubość ramek, poświatę i charakter animacji. Wybór działa od razu, bez ponownego uruchamiania.</li>" +
			"</ul>" +
	...
	settingsStrings["uk"] = map[string]string{
		...
		"S_LLM_HINT": "Позначені профілі застосовуються по черзі, зверху вниз, під час звичайного диктування. Нічого не позначено — текст вставляється як є. Хоткей профілю застосовує разово лише його.",
		...
		"S_HELP_HTML": "<p class=\"wh\">Як це працює</p>" +
			...
			"<li>Вигляд усього застосунку — вікна, смужки, вікна сполучення та значка в лотку — задається одним списком «Оформлення» в розділі «Система». Виглядів шість: зелений термінальний за замовчуванням, бурштиновий, синій, рожевий, «Редактор» (плаский сірий, без сяйва) і «Неон» (фіолетовий, зі скругленнями). Вигляд змінює не лише колір: свій шрифт, скруглення, товщину рамок, сяйво та характер анімації. Вибір діє одразу, без перезапуску.</li>" +
			"</ul>" +
