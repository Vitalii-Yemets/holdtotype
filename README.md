...
- 🎨 **Design and colour, kept apart** — seven designs, all set in one face, IBM Plex Mono, carried inside the exe so they read the same on any machine. Editor is the default: dark grey with a blue accent, filled blue buttons for the main action of each section, grey for the rest, red for the destructive ones. Fluent follows the Windows 11 settings app — light-blue accent, a toggle that fills with the accent and carries a dark knob. Studio is graphite with orange, corners of three and four pixels, the look of an audio editor. Soft stands on light lilac with pill-shaped buttons and a purple toggle, Document on paper with blue kept for the main action and a two-pixel line on top of every card. Neon is violet: its main buttons take a pink-to-cyan gradient, its badges are pink, and the card with the shortcut is framed by the same gradient. Terminal is green on black with brackets around the buttons, square corners and a halo on the text. Each design sets its own corner radius for buttons, fields, cards and key chips, the colours of its toggle in both states, the colours of its badges and of the shortcut keys, the way the level meter moves and what the pill does the moment the text lands: Terminal blinks, Neon flares, Soft bounces, the rest stay still. The window takes the corners its design asks for — rounded, small for Studio, square for Terminal — and a light design carries its light through everything the program draws: the title bar Windows itself paints, the plate, the tray menu, the tray icon and the installer. Colour is offered to Terminal alone — green, amber, blue, pink — and touches nothing but the colour of the window, the plate and the tray icon; the other designs bring their own. Either choice repaints the whole program at once, with no restart. The installer follows the design of the copy it is updating, down to its buttons, toggles and window corners.
- 🗣️ **8 UI languages** — English, Ukrainian, German, French, Spanish, Italian, Polish, Russian. Everything is translated: screens, dialogs, the overlay, the tray, the uninstaller and the in-app guide. Switching is instant, "Same as system" follows Windows.
...
On the plate the target language comes first and the rest follow in the settings order, so the language you use most is always under the same finger. The last button, **Translate into the source language**, hands back what was actually said.
...
| [IBM Plex](https://github.com/IBM/plex) | Mono, the one face every design is set in, carried inside the exe | SIL Open Font License 1.1 |
