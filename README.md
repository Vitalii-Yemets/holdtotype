...
Left-click the tray icon to open the settings window. Twelve sections in four groups — General, Speech, Text, Info — and no tabs inside tabs:

| Section | Contents |
|---|---|
| **Overview** | is everything ready, at a glance: hotkey, microphone with a level meter the width of its card, the model for Russian and the one for every other language, the post-processing model, free memory, and the last dictation with *Copy*. When the recognizer will not start, this screen says why and offers *Try again* — no app restart |
...
| **Dictation** | the hotkey, hold or toggle, the recognition model with its language right beside the shortcut they serve, auto-Enter, the on-screen overlay; folded away: the pause shortcut, recording durations, clipboard restore, character-by-character typing, overlay animation |
| **Microphone** | input device with a live level meter, recording cue sounds and the sound theme |
| **Models** | the library: what is installed and what can be downloaded, recognition models and the editor model for post-processing alike (installed + Hugging Face search). Every row carries its languages, its translate ability and an honest RAM number; the active model wears a pill and the standby one another, and a click on a row is the choice. A language filter and a name search stand on top, next to the picker that answers with a model and the reason, and the button that re-checks installed files against their reference hashes |
...
| **Post-processing** | the chain of editing prompts with their shortcuts, on a page of its own instead of the bottom of an expert fold |
...
| **About** | the version and what the app is made of |
| **Contacts** | about the author and where to write |
