...
- ⚡ **Live text while you speak** — assign Nemotron 3.5 (40 languages, punctuates and capitalizes by itself) to a language, and the words appear on the plate as you say them, refreshed a few times a second. The plate takes a fixed three-line window the moment the stream opens and never jumps around: the phrase grows at the bottom and the older lines slide up out of view under the control row, the way Handy draws it. The control row itself carries the recording dot, a running time counter, the voice level and the ✕. Release the key and the finished phrase goes through the same replacements, prompts and insertion as any other dictation. Pauses do not lose text: the phrase is stitched together across them. When the stream cannot start, the dictation falls back to ordinary recognition on the same model — nothing is lost.
...
- 🔤 **Replacements after recognition** — a list of what the model mishears and what it should become: `git hub` → GitHub, surnames, in-house terms. Each rule serves every language or is pinned to one, so a Russian fix never mangles an English dictation. They run right after recognition and before the prompts, match whole words and ignore case by default, and a field right there tries them on any phrase without dictating.
...
| Section | Contents |
|---|---|
| **Status** | is everything ready, at a glance: hotkey, microphone with a level meter the width of its card, the model behind the current language, the post-processing model, free memory — with the models actually sitting in it named right beside, so the number means something — and the last dictation with *Copy* and an honest word about how it sounded: level fine, too quiet, clipped. Below: every model in use with the languages it serves, the models installed locally, and a line of the week — how many dictations, how many characters, counted from the history that never leaves the machine. When the recognizer will not start, this screen says why and offers *Try again* — no app restart |
| **History** | the switch that turns it on, how long entries are kept and how many, the programs nothing is ever recorded from — added with a button and worn as removable chips — search, and per entry: copy, paste back into the window you came from, and clearing everything |
...
| **Languages & models** | which model serves which language — a preset row per language, Whisper Medium for all of them out of the box, nothing forced. Below, the library: every model the app knows, in any state — installed, downloadable, or "download it yourself and drop it into the models folder" where a licence forbids redistribution — plus any model you copy into `models/` by hand, found after a restart and shown honestly without bars, since its powers are unknown. Every card carries its languages, its translate ability (or the words "recognition only"), an honest RAM number and the plain line that everything runs on the CPU; the model serving the current language wears a pill, cards assigned to languages name them, the loaded one offers to leave the memory, and a click on a card assigns it to the current language. The editor model for post-processing lives here too (installed + Hugging Face search, filtered by default to files that fit this computer, with the hidden ones counted). A language filter and a name search stand on top, next to the picker and the button that re-checks installed files against their reference hashes |
...
| **Post-processing** | the chain of editing prompts with their shortcuts, on a page of its own |
...
| **Help** | the detailed guide-wiki with its table of contents |
| **About** | the version, what the app is made of, and the repository |
| **Contacts** | about the author and the mail address to write to |
