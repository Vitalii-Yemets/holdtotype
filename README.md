...
| Section | Contents |
|---|---|
| **Status** | is everything ready, at a glance: hotkey, microphone with a level meter the width of its card, the model behind the current language, the post-processing model, free memory, and the last dictation with *Copy*. Below: every model in use with the languages it serves, and the models installed locally. When the recognizer will not start, this screen says why and offers *Try again* — no app restart |
