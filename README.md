...
- 🖥️ **Tray application** — a status icon whose failure state carries a badge instead of yet another shade of grey, a quick menu that sizes itself to the language it is written in and comes back after Explorer restarts, a Pip-Boy-terminal-styled settings window that remembers its size.
