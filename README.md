...
- 📟 **On-screen overlay** — one pill at the bottom of the screen and nothing else: live voice level while recording, processing stages, the result, and — on a second line when needed — the questions (which language to translate into, where to insert when focus changed) with mouse or keyboard answers. The ✕ or Esc cancels at any stage; input focus is never stolen.
